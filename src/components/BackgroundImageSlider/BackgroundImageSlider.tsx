...
const getRandomZoom = () =>
  zoomEffects[Math.floor(Math.random() * zoomEffects.length)];
  ...
  return (
    <div className="background-slider">
      {images.map((src, index) => {
        ...
        const shouldAnimate =
          isActive || isPrevious || (!hasStarted && index === 0);

        const effectClass =
          shouldAnimate && (hasStarted || index !== 0)
            ? effects[index]
            : "";
        ...
        return (
          <div
            key={index}
            className={`background-slider__slide ${isActive ? "active" : ""} ${
              isPrevious ? "previous" : ""
            } ${effectClass}`}
          >
